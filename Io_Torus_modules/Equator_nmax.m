...
%This code runs a version of nmax (Lat_dist application) to produce density values along the
%equator. We extrapolate along the field line to the maximum densities, and
%fill the n_max_{species} list w/ each value.
%I recommend reading the physics walkthrough, and at least understand what
%each paramater is in each force


%preparing electron parameters to be used in Lat Dist
tec_list = interp1(L_e,ColdTemperatureSS,L_inbound);
teh_list = interp1(L_e,HotTemperatureSS,L_inbound);
nhnc_list = interp1(L_e,HotColdDensitySS,L_inbound);




%preparing arrays of zeroes to be filled with

        n_max_Oxygen = zeros(1,length(Rad));
        n_max_Oxygen1 = zeros(1,length(Rad));
        n_max_HotOxygen = zeros(1,length(Rad));
        n_max_Sulfur = zeros(1,length(Rad));
        n_max_Sulfur1 = zeros(1,length(Rad));
        n_max_Sulfur2 = zeros(1,length(Rad));
        n_max_Hydrogen = zeros(1,length(Rad));
        n_max_Sodium = zeros(1,length(Rad));
        n_max_Electron = zeros(1,length(Rad));


for counter = 1:length(Rad) %length of each list to be iterated, i.e. the
                               %number of extrapolations
    
    ns=8; %number of species to be considered
    posR = r_cent_location_Lmapped(counter); 
    posT = 90 - theta_sys3_cent_location_Lmapped(counter);
    scposR = Rad(counter);
    scposT = 90 - SIII_Theta(counter);
    %tpar=64; I assign this value inside the second nested for loop.
    te = tec_list(counter);% this is a list that I create in the te_teh_nhnc_calculator  module
    mass=[16,16,16,32,32,32,1,23];
    Charge=[1 2 1 1 2 3 1 1];
    fcorot=Fractional_Corotation(counter);
    teh=teh_list(counter);% this is a list that I create in the te_teh_nhnc_calculator  module
    nhnc=nhnc_list(counter); %this is a list that I create in the te_teh_nhnc_calculator  module
    ...
    for i = 1:20 %The squeeze is typically solved in < 20 iterations
        ...
        %assumption
        df = 0 ; % it is used to increment the squeezing process
