load('/Users/pahi9557/Documents/MATLAB/Alfven_Waves/AW_data_prep/sorted_lines.mat')
...
VIP_Cassini_UVIS_variation%This module,located in sys_3_variations/sys_3_torus_modules,
...
progress1 = 0;
    ...
    for counter = 1:length(sorted_lines_1_360.theta) %length of each list to be iterated
      ...
      scpos_electron_density = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Oxygen,scposR)+2*interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Oxygen1,scposR)+interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_HotOxygen,scposR)...
            +interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur,scposR)+2*interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur1,scposR)+3*interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur2,scposR)...
            +interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Hydrogen,scposR)+interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sodium,scposR);
        ...
        for squeeze = 1:21 %The squeeze is typically solved in < 20 iterations
            ...
            for species_counter = 1:ns % This is the loop where we will finish calculating F-centrifugal.
                ...
                if species_counter == 1
                    ...
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Oxygen,scposR);
                end
                
                if species_counter == 2
                    tpar = interp1(M_inbound,OTemperature1,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Oxygen1,scposR);
                end
                
                if species_counter == 3
                    
                    tpar = interp1(M_inbound,HotOTemperature,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_HotOxygen,scposR);
                end
                
                if species_counter == 4
                    
                    tpar = interp1(M_inbound,STemperature,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur,scposR);
                end
                
                if species_counter == 5
                    tpar = interp1(M_inbound,STemperature1,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur1,scposR);
                end
                
                if species_counter == 6
                    tpar = interp1(M_inbound,STemperature2,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sulfur2,scposR);
                end
                
                if species_counter == 7
                    
                    tpar = interp1(M_inbound,HTemperature,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Hydrogen,scposR);
                end
                
                if species_counter == 8
                    
                    tpar = interp1(M_inbound,NaTemperature,6);
                    n_scpos = interp1(VIP_cent_loc.r,VIP_nmax_eq.n_max_Sodium,scposR);
                end
...
% figure()
% grid on
% semilogy(field_line_68.Theta,HotO_grid,'Linewidth',4);
% hold on
% semilogy(field_line_68.Theta,H_grid,'Linewidth',4);
% semilogy(field_line_68.Theta,O_grid,'Linewidth',4);
% semilogy(field_line_68.Theta,O1_grid,'Linewidth',4);
% semilogy(field_line_68.Theta,Na_grid,'Linewidth',4);
% semilogy(field_line_68.Theta,S_grid,'Linewidth',4);
% semilogy(field_line_68.Theta,S1_grid,'Linewidth',4);
% i = semilogy(field_line_68.Theta,S2_grid,'Linewidth',4);
...
% set(gca,'FontSize',16)
% title('M = 6 \phi = 68','fontweight','bold','fontsize',22)
